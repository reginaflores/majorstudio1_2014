...
//setting up default variables
    ...
    gui->addToggle("Balls On", &circleOn);
    gui->addToggle("Shader On", &cloudsOn);
    gui->addToggle("Lines", &linesOn);
    ...
    fbo.allocate(secondWindow.getWidth(), secondWindow.getHeight());
}
...
void ofApp::draw(){
    ...
    for(int i = 0; i < contourFinder.size(); i++) {
      ...
      /////////////////////////////////////////
        ...
        float x = ofMap(pp.x, 1, 0, 0, secondWindow.getWidth());
        float y = ofMap(pp.y, 0, 1, 0, secondWindow.getHeight());
            
            if (linesOn) {
                ofSetColor(255, 80);
                //ofLine(x, y,  ofGetMouseX(),ofGetMouseY());
                
                ofLine(x, y, secondWindow.getWidth()/2, y);
                ofLine(x, y, x, secondWindow.getHeight()/2);
            }
